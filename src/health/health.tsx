...
//Fix Medication Card placements
...
export const AddAmedicationTab = ({setPage} : AddAMedicineProps) => {
...
export const MedicationCard = () => {
  const [medicationName, setMedicationName] = useState('Oxycodone');
  const [medicationDosage, setMedicationDosage] = useState('5');
  //fix the date issue, only hours required
  const [medicationTime, setMedicationTime] = useState('10:30');
  const [status, setStatus] = useState('');
  useEffect(() => {
    const currentDate = new Date();
    const medicationDate = new Date(medicationTime);
    if (medicationDate < currentDate) {
      setStatus('Pending');
    } else {
      setStatus('Taken');
    }
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {`${medicationName} - ${medicationDosage} mg`}
            </Box>
            <Box
              sx={{
                marginTop: 0.75,
...
interface HealthReiewProps {
  setPage: (page: string) => void;
}

export const HealthReview = ({setPage} : HealthReiewProps) => {
  return (
